 ...
 *
 * <p>In some variations of the {@code rotate} methods in the {@code AffineTransform} class, a
 ...
 *
 * <p>Since 90 degrees is represented as {@code PI/2} in radians, and since PI is a transcendental
 ...
 * Double precision floating point values can get very close to non-zero multiples of {@code PI/2}
 * but never close enough for the sine or cosine to be exactly 0.0, 1.0 or -1.0. The implementations
 * of {@code Math.sin()} and {@code Math.cos()} correspondingly never return 0.0 for any case other
 * than {@code Math.sin(0.0)}. These same implementations do, however, return exactly 1.0 and -1.0
 * for some range of numbers around each multiple of 90 degrees since the correct answer is so close
 * to 1.0 or -1.0 that the double precision significand cannot represent the difference as
 * accurately as it can for numbers that are near 0.0.
 *
 * <p>The net result of these issues is that if the {@code Math.sin()} and {@code Math.cos()}
 ...
 * quadrant rotation even for a simple case like {@code rotate(Math.PI/2.0)}, due to minor
...
public class AffineTransform implements Cloneable, java.io.Serializable {
     ...
     *     AffineTransform} object. The length of the array is assumed to be at least 4. If the
     *     length of the array is less than 6, only the first 4 values are taken. If the length of
    ...
    /**
     * Constructs a new {@code AffineTransform} from an array of double precision values
     ...
     * @return an {@code AffineTransform} object that represents a translation transformation,
     ...
     * coordinates. If both {@code vecx} and {@code vecy} are 0.0, an identity transform is
     ...
     * @return an {@code AffineTransform} object that shears coordinates by the specified
     *     multipliers.
     * @since 1.2
     */
     ...
     * combination of the appropriate flag bits. A valid combination of flag bits is an exclusive OR
     * operation that can combine the TYPE_TRANSLATION flag bit in addition to either of the
     ...
     *
     * <p>If this transform represents a uniform scale, as indicated by the {@code getType} method
    ...
    /**
     * Returns the {@code m00} element of the 3x3 affine transformation matrix. This matrix factor
     * determines how input X coordinates will affect output X coordinates and is one element of the
     * scale of the transform. To measure the full amount by which X coordinates are stretched or
     ...
     * </pre>
     *
     * @return a double value that is {@code m00} element of the 3x3 affine transformation matrix.
     * @see #getMatrix
     * @since 1.2
     */
    ...
    /**
     * Returns the {@code m11} element of the 3x3 affine transformation matrix. This matrix factor
     * determines how input Y coordinates will affect output Y coordinates and is one element of the
     * scale of the transform. To measure the full amount by which Y coordinates are stretched or
     * contracted by this transform, use the following code:
     *
     * <pre>
     ...
     * </pre>
     *
     * @return a double value that is {@code m11} element of the 3x3 affine transformation matrix.
     * @see #getMatrix
     * @since 1.2
     */
     ...
     * vector pointing from the origin to the specified vector coordinates. If both {@code vecx} and
     * {@code vecy} are 0.0, no additional rotation is added to this transform. This operation is
     ...
     * vector coordinates. If both {@code vecx} and {@code vecy} are 0.0, the transform is not
     ...
     * with the vector pointing from the origin to the specified vector coordinates. If both {@code
     * vecx} and {@code vecy} are 0.0, the transform is set to an identity transform. This operation
     ...
     * the specified vector coordinates. If both {@code vecx} and {@code vecy} are 0.0, the
    ...
    /**
     * Concatenates an {@code AffineTransform Tx} to this {@code AffineTransform} Cx in the most
     * commonly useful way to provide a new user space that is mapped to the former user space by
     * {@code Tx}. Cx is updated to perform the combined transformation. Transforming a point p by
     * the updated transform Cx' is equivalent to first transforming p by {@code Tx} and then
     * transforming the result by the original transform Cx like this: Cx'(p) = Cx(Tx(p)) In matrix
     * notation, if this transform Cx is represented by the matrix [this] and {@code Tx} is
    ...
    /**
     * Concatenates an {@code AffineTransform Tx} to this {@code AffineTransform} Cx in a less
     * commonly used way such that {@code Tx} modifies the coordinate transformation relative to the
     * absolute pixel space rather than relative to the existing user space. Cx is updated to
     * perform the combined transformation. Transforming a point p by the updated transform Cx' is
     * equivalent to first transforming p by the original transform Cx and then transforming the
     * result by {@code Tx} like this: Cx'(p) = Tx(Cx(p)) In matrix notation, if this transform Cx
     * is represented by the matrix [this] and {@code Tx} is represented by the matrix [Tx] then
    ...
    /**
     * Returns an {@code AffineTransform} object representing the inverse transformation. The
     ...
     * inverse mapping. The {@code getDeterminant} method can be used to determine if this transform
     * has no inverse, in which case an exception will be thrown if the {@code createInverse} method
     * is called.
     *
     ...
     * inverse, since coordinates that do not lie on the destination point or line will not have an
     * inverse mapping. The {@code getDeterminant} method can be used to determine if this transform
     * has no inverse, in which case an exception will be thrown if the {@code invert} method is
     * called.
     *
     * @see #getDeterminant
     * @exception NoninvertibleTransformException if the matrix cannot be inverted.
     ...
     * results into {@code Point2D} objects that will be used as the source for calculations further
     * down the source array. This method does guarantee that if a specified {@code Point2D} object
     * is both the source and destination for the same single point transform operation then the
     * results will not be stored until the calculations are complete to avoid storing the results
     * on top of the operands. If, however, the destination {@code Point2D} object for one operation
     * is the same object as the source {@code Point2D} object for another operation further down
     * the source array then the original coordinates in that point are overwritten before they can
     * be converted.
     *
     ...
     * overwritten by a previous operation before they can be transformed. The coordinates are
     * stored in the arrays starting at the specified offset in the order {@code [x0, y0, x1, y1,
     * ..., xn, yn]}.
     *
     * @param srcPts the array containing the source point coordinates. Each point is stored as a
     ...
     * stored in the arrays starting at the indicated offset in the order {@code [x0, y0, x1, y1,
     * ..., xn, yn]}.
     *
     * @param srcPts the array containing the source point coordinates. Each point is stored as a
     *     pair of x,&nbsp;y coordinates.
    ...
    /**
     * Transforms an array of floating point coordinates by this transform and stores the results
     * into an array of doubles. The coordinates are stored in the arrays starting at the specified
     ...
     * {@code ptDst} is {@code null}, a new {@code Point2D} object is allocated and then the result
     ...
     * </pre>
     *
     * If {@code ptDst} is {@code null}, a new {@code Point2D} object is allocated and then the
     * result of the transform is stored in this object. In either case, {@code ptDst}, which
     * contains the transformed point, is returned for convenience. If {@code ptSrc} and {@code
     * ptDst} are the same object, the input point is correctly overwritten with the transformed
     * point.
     *
    ...
    /**
     * Transforms an array of relative distance vectors by this transform. A relative distance
     * vector is transformed without applying the translation components of the affine
     ...
     * </pre>
     *
     * The two coordinate array sections can be exactly the same or can be overlapping sections of
     * the same array without affecting the validity of the results. This method ensures that no
     * source coordinates are overwritten by a previous operation before they can be transformed.
     * The coordinates are stored in the arrays starting at the indicated offset in the order {@code
     * [x0, y0, x1, y1, ..., xn, yn]}.
     *
     * @param srcPts the array containing the source distance vectors. Each vector is stored as a
     *     pair of relative x,&nbsp;y coordinates.
     ...
     * @param dstOff the offset to the location of the first transformed vector that is stored in
    ...
    /**
     * Returns a new {@link Shape} object defined by the geometry of the specified {@code Shape}
     ...
     * @return a new {@code Shape} object that defines the geometry of the transformed {@code
     *     Shape}, or null if {@code pSrc} is null.
     * @since 1.2
     */
