...
public class NADCONGridShiftFactory extends ReferencingFactory implements BufferedFactory {
    ...
    private NADConGridShift loadBinaryGrid(final URL latGridUrl, final URL longGridUrl)
        ...
        try {
            ...
            ((Buffer) latBuffer).position(((Buffer) latBuffer).position() + DESCRIPTION_LENGTH);
            ...
            ((Buffer) longBuffer).position(((Buffer) longBuffer).position() + DESCRIPTION_LENGTH);
            ...
            for (i = 0; i < nr; i++) {
                ((Buffer) latBuffer)
                        .position(
                                ((Buffer) latBuffer).position()
                                        + SEPARATOR_BYTES); // skip record separator
                ((Buffer) longBuffer).position(((Buffer) longBuffer).position() + SEPARATOR_BYTES);
